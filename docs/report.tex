...
\author{
    Will Frey\thanks{Thanks to Dave Wells for providing me with mesh parsing
        software and teaching me how to use gmsh. Thanks also for the endless
    Borggaard for the ``rules of thumb'' and other guidance. Thanks also to
Jeff Borggaard for his helpful ``rules of thumb.''}
}%
...
\begin{abstract}
    NOT FINISHED
    A mixed finite element method is derived for the steady state Stokes flow
    equation. Derivation of the weak form solution is provided and the
    corresponding system is written in a system of linear equations.
...
\begin{keywords}
    steady stokes flow, finite element method, mixed finite elements
\end{keywords}

\pagestyle{myheadings}
\thispagestyle{plain}
\markboth{WILL FREY}{MIXED METHODS FOR STEADY STOKES FLOW}

\section{Introduction}
The Stokes equation (or \textit{Stokes flow}) is derived by making certain
simplifying assumptions and applying them to the non-linear Navier-Stokes
equations. The flow of an incompressible viscous fluid in an $n$-dimensional
domain (with $n = 2$ or $3$)\cite{br}:
\begin{subequations}\label{Stk}
\begin{equation}
    \nabla^2 u + \nabla p =& -f \quad \text{in } \Omega,
\end{equation}
\begin{equation}
    \nabla \dot u =& 0 \quad \text{in } \Omega,
\end{equation}
\begin{equation}
    u =& u_0 \quad \text{on } \p \Omega.
\end{equation}
\end{subequations}
...
The Stokes Equation, otherwise known as \textit{Stokes Flow}, is the linearized
...
\begin{definition}
    ...
    \begin{thebibliography}{10}
        \bibitem{gr}{\sc Vivette Girault and Pierre-Arnaud Raviart}
            {\em Finite Element Method for Navier-Stokes Equations: Theory
            and Algorithms}, Springer-Verlag, Berlin, Germany, 1986.

        \bibitem{br}{\sc Finite elements: Theory, fast solvers, and
